...
if __name__ == "__main__":
    ...
    for g in range(1):
        ...
        if start != 0:
            ...
            for flag in flags:
                ...
                if charudian != -1:
                    ...
                    while charudian < len(final) and pos1 < len(seq):
                        if information_new[seq[pos1]][3] == 1:
                            # 一个也没插入四驱车
                            diaoyong_flag = 1
                            break
            ...
            if B_siqu_len % 2 == 1 and B_luan_siqu_len % 2 == 1:
                ...
                if B_shihei_shanyao_len == 2:
                    ...
                    b = new_duan[4][5]
                    new_duan[4].pop(5)
                    new_duan[4].insert(0, new_duan[4][-1])  # 插一个二驱到最前面
            ...
            for i in range(len(new_duan[6]) - 1, -1, -1):
                if information_new[new_duan[6][i]][1] == 8 and information_new[new_duan[6][i]][3] == 1:
                    a = new_duan[6][i]
                    b = new_duan[6][i - 1]
                    c = new_duan[6][i - 2]
            ...
            if B_luan_siqu_len % 2 == 1 and B_siqu_len % 2 == 0 and A_siqu_len % 2 == 1 and tansuo_flag != 1:
                print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa2")  # 1
            # B闪耀黑和A不同颜色车各自多了一个四驱车(这样可能会减少连续生产五辆颜色相同的闪耀黑)
            ...
            if flag == 1:
                ...
                seq = convert_two_to_one(duan)
                if check_solution(seq, start) != 1:
                    sys.exit("序列出错了q！oi7poqw9800921")
                results.append(duan)  # 增加一个解
            ...
            # A不同颜色车和B闪耀黑各自多了一个四驱车(这样可能会破坏连续生产五辆相同颜色的闪耀黑)
                ...
                for i in range(len(temp_duan[4]) - 1, -1, -1):
                    if information_new[temp_duan[4][i]][3] == 1:
                        ...
                        break
                temp_duan[5], temp_duan[6] = temp_duan[6], temp_duan[5]  # 交换位置
                connect(temp_duan, information_new, start, results)   #原本连接方式为：B天空灰B水晶B液态B乱B闪耀A乱A闪耀A水晶A天空
            # B不同颜色车和A不同颜色车各自多了一个四驱车(这样可能会减少连续生产五辆颜色相同的闪耀黑)
                ...
                temp_duan[3], temp_duan[4] = temp_duan[4], temp_duan[3]
                temp_duan[5], temp_duan[6] = temp_duan[6], temp_duan[5]
                connect_other(temp_duan, information_new, start, results)  # 这里连接方式的bug应该修复了
            # B不同颜色车和A闪耀黑各自多了一个四驱车(这样可能会减少连续生产五辆颜色相同的闪耀黑)
                ...
                for i in range(len(temp_duan[5]) - 1, -1, -1):
                    if information_new[temp_duan[5][i]][3] == 1:
                        ...
                        temp_duan[5].pop(i + 1)  # 删除四驱车
                        break
                connect2(temp_duan, information_new, start, results) # 这里连接方式的bug应该修复了
            else:
                connect(temp_duan, information_new, start, results)
        ...
        if diaoyong_flag != 0:
            pop_other = data_373_b(start)
            # pop_other_new = data_373_b_new(start)
            # pop_new = pop_other_new
            # pop_other1 = data_373_c(start)
            # pop_other2 = data_373_d(start)
            pop_new.extend(pop_other)
            # pop_new.extend(pop_other_new)
            # pop_new.extend(pop_other1)
        ...
        if len(final_sequences) > 50:
            print("解超过五十个了")

        # np.savetxt("./data_"+ str(start)+ "_archive_VAR.txt", np.array(final_sequences), fmt='%d')
        ...
        # print(pop_new.population[0].objectives)
        # print_solution(pop_new.population[0], problem)

        # final = np.array([final]).astype(int)
        # np.savetxt(str(start) + ".txt", final_sequences, delimiter=" ", fmt='%d')
